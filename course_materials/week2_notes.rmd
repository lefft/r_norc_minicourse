...
`rownames(tim_mtcars)` is just a character vector, let's move it to a column, and then delete the rownames.
...
length(is.na(tim_mtcars$mpg))
length(is.na(tim_mtcars$cyl))
length(is.na(tim_mtcars$disp))
# ...
```
...
**exercise**: use the `%in%` function to see whether the column `mtcars$wt` has any missing values. Note that `%in%` has **infix** syntax like `+`, `-`, `*`, and `/`, so we write `x %in% y` to see if `y` contains `x`. We'll see another useful infix operator below...
...
#### 3.5 cleaning data and then writing/saving it
...
```{r}
mfg_NA   <- c("Hornet", "Valiant", "Duster")           # unknown  manufacturer
mfg_asia <- c("Mazda", "Datsun", "Honda", "Toyota")    # asian    manufacturer
mfg_usa  <- c("Cadillac","Lincoln","Chrysler","Dodge", # american manufacturer
              "AMC","Chevrolet","Pontiac","Ford")      # european manufacturer
mfg_euro <- c("Mercedes", "Fiat", "Porsche", "Lotus", 
...
Make the four `mfg_*` vectors into a single object...But how should we do *that*? A natural instinct might be to make each of them a column of a data frame...

```{r}
# but if we try this we'll get an error -- why?
# car_regions <- data.frame(mfg_NA, mfg_asia, mfg_usa, mfg_euro)
```
...
car_regions <- data.frame(
  # the mfg_* vectors strung together
  make   = c(mfg_NA, mfg_asia, mfg_usa, mfg_euro), 
  # assign regions to manufacturers, based on the mfg_* vectors and 'make'
  # the idea is to repeat the label for each value in the corresponding vector
  region = c(rep(NA,    length(mfg_NA)),   rep("asia", length(mfg_asia)), 
             rep("usa", length(mfg_usa)),  rep("euro", length(mfg_euro))),
...
<!-- # match(c("Valiant","Honda"), table=c(mfg_NA, mfg_asia, mfg_usa, mfg_euro)) -->
...
- visualizing your own data with base graphics and `ggplot2::`
...
except that the first one is easier to read. Assuming we want to save the result of `x` applied to `f()` through `z()`, we can just assign the whole chain to a variable. Here's a little example. 
...
to each of them. We could type out `as.character()` each time, or we could write a function that basically just abbreviates it. That'll save us keystrokes.
