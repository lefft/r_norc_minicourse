...
# container- versus pointer-based semantics for variables
...
as "containers" that you can dump an arbitrarily large/complex object into. 
But in Python, objects kinda just chill on a shelf in memory, so Python 
variables are more like "pointers" to an object that exists independently of 
how we refer to it. (this analogy is approximate!)
...
This demo also illustrates a badass property of R Markdown, for which we have [Yihui Xie](https://twitter.com/xieyihui?lang=en)'s `knitr::` package to thank! The property is that you can use lots of different 
languages inside of code chunks (with appropriate chunk options), provided 
that you have an interpreter or compiler for that language installed on your machine. 
To learn more, first read up about `knitr::` in general, then 
read this page of the documentation. (btw, the `<WORD>::` syntax means that 
`<WORD>` is an R package. we'll see why this syntax is nice later on)



# an older tutorial -- might be useful
